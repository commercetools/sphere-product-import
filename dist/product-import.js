var CommonUtils, EnsureDefaultAttributes, EnumValidator, Mutex, ProductImport, ProductSync, Promise, Reassignment, Repeater, SphereClient, UnknownAttributesFilter, _, debug, fs, path, ref1, serializeError, slugify, util,
...
ProductImport = (function() {
  function ProductImport(logger1, options) {
    ...
    if (this.variantReassignmentOptions.enabled) {
      this.reassignmentLock = new Mutex();
      this.reassignmentTriggerActions = ['removeVariant'];
      ...
      return function(productsToProcess) {
        ...
        return _this._ensureProductTypesInMemory(productsToProcess).then(function() {
          ...
          if (filteredProductsLength) {
            ...
            return _this._summary.productsWithMissingSKU += filteredProductsLength;
          }
        }).then(function() {
    ...
    return new Promise((function(_this) {
      return function(resolve, reject) {
        ...
        return Promise.map(skuChunks, function(skus) {
          ...
          uniqueProducts = _.uniq(_.flatten(results), function(product) {
            return product.id;
          });
          return resolve(uniqueProducts);
        })["catch"](function(err) {
          return reject(err);
        });
      };
  ...
  ProductImport.prototype._errorLogger = function(res, logger) {
    if (this._summary.failed < this.errorLimit || this.errorLimit === 0) {
      return logger.error(util.inspect(res, false, 3, true), "Skipping product due to an error");
    } else if (!this.errorLimitHasBeenCommunicated) {
      this.errorLimitHasBeenCommunicated = true;
  ...
  ProductImport.prototype._handleFulfilledResponse = function(res) {
    var ref2;
    switch ((ref2 = res.value()) != null ? ref2.statusCode : void 0) {
      case 201:
  ...
  ProductImport.prototype._getProductsMatchingByProductSkus = function(prodToProcess, existingProducts) {
    var prodToProcessSkus;
    prodToProcessSkus = this._extractUniqueSkus([prodToProcess]);
    return _.filter(existingProducts, (function(_this) {
  ...
  ProductImport.prototype._updateProductRepeater = function(prodToProcess, existingProducts, reassignmentRetries) {
    var repeater;
    ...
    return repeater.execute((function(_this) {
      return function() {
        return _this._updateProduct(prodToProcess, existingProducts, false, reassignmentRetries);
      };
    })(this), (function(_this) {
      return function(e) {
        if (e.statusCode !== 409) {
        ...
        _this.logger.warn("Recovering from 409 concurrentModification error on product '" + existingProducts[0].id + "'");
        return Promise.resolve(function() {
          return _this.client.productProjections.staged(true).byId(existingProducts[0].id).fetch().then(function(result) {
            return _this._updateProduct(prodToProcess, [result.body], true, reassignmentRetries);
          });
        });
      };
    })(this));
  ...
  ProductImport.prototype._getProductTypeIdByName = function(name) {
    return this._resolveReference(this.client.productTypes, 'productType', {
      id: name
    }, "name=\"" + name + "\"");
  };

  ProductImport.prototype._runReassignmentForProduct = function(product) {
    this.logger.info("Running reassignment for a product with masterVariant.sku \"" + product.masterVariant.sku + "\" - locking");
    return this.reassignmentLock.lock().then((function(_this) {
      return function(unlock) {
        return _this.reassignmentService.execute([product], _this._cache.productType)["finally"](function() {
          _this.logger.info("Finished reassignment for a product with masterVariant.sku \"" + product.masterVariant.sku + "\" - unlocking");
          return unlock();
        });
      };
    })(this));
  ...
  ProductImport.prototype._runReassignmentBeforeCreateOrUpdate = function(product, reassignmentRetries) {
    var masterSku;
    if (reassignmentRetries == null) {
      reassignmentRetries = 0;
    }
    if (reassignmentRetries > 5) {
      masterSku = product.masterVariant.sku;
      return Promise.reject(new Error("Error - too many reassignment retries for a product with masterVariant.sku \"" + masterSku + "\""));
    }
    return this._runReassignmentForProduct(product).then((function(_this) {
      return function(res) {
        if (res.badRequestSKUs.length) {
          _this.logger.error("Removing product with SKUs \"" + res.badRequestSKUs + "\" from processing due to a reassignment error");
          return;
        }
        return _this._createOrUpdateProduct(product, null, ++reassignmentRetries);
      };
    })(this));
  };

  ProductImport.prototype._updateProduct = function(prodToProcess, existingProducts, productIsPrepared, reassignmentRetries) {
    var existingProduct;
    existingProduct = existingProducts[0];
    return Promise.all([this._fetchSameForAllAttributesOfProductType(prodToProcess.productType), productIsPrepared ? prodToProcess : this._prepareUpdateProduct(prodToProcess, existingProduct), this._getProductTypeIdByName(prodToProcess.productType.id)]).then((function(_this) {
      return function(arg) {
        var hasProductTypeChanged, newProductTypeId, preparedProduct, sameForAllAttributes, shouldRunReassignment, synced, updateRequest;
        sameForAllAttributes = arg[0], preparedProduct = arg[1], newProductTypeId = arg[2];
        ...
        if (!((hasProductTypeChanged && _this.variantReassignmentOptions.enabled) || synced.shouldUpdate())) {
        ...
        if (_this.variantReassignmentOptions.enabled) {
          shouldRunReassignment = existingProducts.length > 1 || hasProductTypeChanged || updateRequest.actions.find(function(action) {
            var ref2;
            return ref2 = action.action, indexOf.call(_this.reassignmentTriggerActions, ref2) >= 0;
          });
          if (shouldRunReassignment) {
            return _this._runReassignmentBeforeCreateOrUpdate(prodToProcess, reassignmentRetries);
          }
        }
        return _this._updateInBatches(synced.getUpdateId(), updateRequest);
      };
    })(this));
  };
  ...
  ProductImport.prototype._canErrorBeFixedByReassignment = function(err) {
    var ref2;
    return ((err != null ? (ref2 = err.body) != null ? ref2.errors : void 0 : void 0) || []).find(function(error) {
      return error.code === 'DuplicateField' && (error.field.indexOf('slug' >= 0 || error.field.indexOf('sku' >= 0)));
    });
  };

  ProductImport.prototype._createOrUpdateProduct = function(prodToProcess, existingProducts, reassignmentRetries) {
    if (reassignmentRetries == null) {
      reassignmentRetries = 0;
    }
    return Promise.resolve(existingProducts).then((function(_this) {
      return function(existingProducts) {
        return existingProducts || _this._getExistingProductsForSkus(_this._extractUniqueSkus([prodToProcess]));
      };
    })(this)).then((function(_this) {
      return function(existingProducts) {
        if (existingProducts.length) {
          return _this._updateProductRepeater(prodToProcess, existingProducts, reassignmentRetries);
        } else {
      ...
      return function(err) {
        if (reassignmentRetries <= 5 && _this.variantReassignmentOptions.enabled && _this._canErrorBeFixedByReassignment(err)) {
          return _this._runReassignmentBeforeCreateOrUpdate(prodToProcess, reassignmentRetries);
        } else {
          throw err;
        }
  ...
  ProductImport.prototype._createOrUpdate = function(productsToProcess, existingProducts) {
    ...
    posts = _.map(productsToProcess, (function(_this) {
      return function(product) {
        return _this._filterAttributes(product).then(function(product) {
          var matchingExistingProducts;
          _this._cleanDuplicateAttributes(product);
          matchingExistingProducts = _this._getProductsMatchingByProductSkus(product, existingProducts);
          return _this._createOrUpdateProduct(product, matchingExistingProducts);
        });
      };
    })(this));
